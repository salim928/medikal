  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active ? "bg-brand-50 text-brand-700" : "text-slate-600 hover:bg-mist hover:text-ink"
      )}
    >
      <Icon className={cn("h-[18px] w-[18px] flex-shrink-0", active ? "text-brand-600" : "text-slate-400 group-hover:text-slate-600")} />
...
export function Sidebar({
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-60 flex-col border-r border-slate-200 bg-white transition-all duration-200",
      ...
      >
        ...
        <div className={cn("flex h-16 items-center px-5", collapsed ? "lg:justify-center lg:px-0" : "justify-between")}>
          <Link href="/" className="flex items-center gap-2" title="medicom">
            ...
            {!collapsed && <span className="font-display text-base font-semibold tracking-tight text-ink">medicom</span>}
          </Link>
          {/* mobile close */}
          <button onClick={onClose} className="rounded-lg p-1.5 text-slate-400 hover:bg-mist lg:hidden">
            <X className="h-5 w-5" />
          </button>
          {/* desktop collapse */}
          {!collapsed && (
            <button onClick={onToggleCollapse} className="hidden rounded-lg p-1.5 text-slate-400 hover:bg-mist hover:text-ink lg:block" title="Collapse sidebar">
        ...
        {collapsed && (
          <button onClick={onToggleCollapse} className="mx-auto mb-1 hidden rounded-lg p-2 text-slate-400 hover:bg-mist hover:text-ink lg:block" title="Expand sidebar">
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-3">
          {!collapsed && <p className="px-3 pb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">Menu</p>}
          {nav.map((item) => (
